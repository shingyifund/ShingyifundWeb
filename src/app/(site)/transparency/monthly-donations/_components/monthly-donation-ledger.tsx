        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 font-serif text-2xl font-black leading-tight text-navy-900">
              {locale === "en" ? "Public Donation Records" : "捐贈公開紀錄"}
            </h2>
          </div>
          <p className="text-sm text-ink-muted">
            {locale === "en" ? "Listed newest first. Select a record to view photos and details." : "依月份由新到舊排列，點選可查看照片與明細。"}
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1">
              <MonthPicker
                ...
                placeholder={locale === "en" ? "Select month" : "選擇年月"}
                className="w-40"
                disabled={isPending}
              />
              {periodValue && (
                <Button
                  ...
                  aria-label={locale === "en" ? "Clear month filter" : "清除年月篩選"}
                >
                  <X className="size-4" />
                </Button>
              )}
      ...
      {total === 0 && !hasFilter ? (
        <div className="rounded-2xl border border-dashed border-navy-200 bg-white p-8 text-center">
          <h2 className="font-serif text-2xl font-bold text-navy-900">
            {locale === "en" ? "No monthly donation records are currently available" : "目前尚無每月捐物清單"}
          </h2>
          <p className="mt-3 text-sm text-muted-foreground">
            {locale === "en" ? "Published records will appear here." : "後台新增資料後，清單會顯示在此頁。"}
          </p>
        </div>
      ) : (
        <>
            ...
            {reports.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="font-serif text-xl font-bold text-navy-900">
                  {locale === "en" ? "No matching records" : "沒有符合條件的紀錄"}
                </p>
                <p className="mt-2 text-sm text-ink-muted">
                  {locale === "en" ? "Change the filters to view other months." : "可以切換篩選條件查看其他月份。"}
                </p>
              </div>
            ) : (
              <div className="divide-y divide-navy-100">
                {reports.map((report) => (
                  <DonationLedgerRow key={report.id} report={report} locale={locale} />
                ))}
              </div>
            )}
          </div>
          ...
          {totalPages > 1 && (
            <div className="mt-4 flex items-center justify-between">
              <p className="text-sm text-ink-muted">
                {locale === "en" ? `${rangeStart}–${rangeEnd} of ${total}` : `第 ${rangeStart}–${rangeEnd} 筆，共 ${total} 筆`}
              </p>
              <div className="flex items-center gap-1">
                <Button
                  variant="outline"
...
function DonationLedgerRow({ report, locale }: { report: MonthlyDonationListItem; locale: "tw" | "en" }) {
