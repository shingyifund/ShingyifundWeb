...
import { getFundraisingReportById } from "../actions";

export default async function EditFundraisingReportPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const report = await getFundraisingReportById(id);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border bg-white p-6">
        ...
        <FundraisingReportForm report={report} />
      </section>
    </div>
  );
}
