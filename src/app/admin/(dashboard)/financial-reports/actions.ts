...
function reportPayload(formData: FormData) {
  ...
  const comparisonYear = parseYear(formData, "comparison_year");

  if (fiscalYear === null) {
    return { error: "請輸入有效主要年度" as const };
  }
...
function revalidateFinancialReports() {
  revalidatePath("/transparency/financial");
  revalidatePath("/");
}
