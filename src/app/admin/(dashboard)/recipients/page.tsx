...
import { RecipientWorkbookImporter } from "./_components/recipient-workbook-importer";
import { DeleteRecipientImportButton } from "./_components/delete-import-button";
...
export default async function RecipientAdminPage() {
  const { rows, ready } = await getImportBatches();
  return (
    <div className="space-y-7">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-700">Public recipients</p>
          <h1 className="mt-1 font-serif text-3xl font-bold text-navy-900">受贈者名單</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-muted">
            上傳後先在瀏覽器解析與去重，確認才以單一交易更新公開資料；原始 Excel 不會保存。
          </p>
        </div>
        <a href="/tw/transparency/recipients" target="_blank" rel="noreferrer" className="text-sm font-semibold text-navy-700 underline decoration-navy-200 underline-offset-4 hover:text-amber-700">
          查看前台受贈者名單 ↗
        </a>
      </header>
      ...
      {!ready ? (
        <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <p>尚未建立受贈者名單資料表。請先到 Supabase SQL Editor 執行本次提供的 SQL。</p>
        </div>
      ) : null}

      <RecipientWorkbookImporter />
              ...
              <article key={batch.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-6">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-ink-muted">
                    ...
                    {" · "}{formatBytes(batch.file_size)}{" · "}{batch.period_count} 個月份
                  </p>
                  <p className="mt-1 text-sm text-ink-soft">{batch.record_count.toLocaleString("zh-TW")} 筆 · {formatRecipientAmount(batch.total_amount)}</p>
                </div>
                <DeleteRecipientImportButton id={batch.id} fileName={batch.file_name} />
              </article>
            ))}
          </div>
        )}
